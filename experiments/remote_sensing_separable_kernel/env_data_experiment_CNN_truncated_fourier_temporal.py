...
from kernels.truncated_fourier import TruncatedFourierExpansion
...
def train_ick(data_generators, input_width, input_height, in_channels, latent_feature_dim, 
    ...
    kernel_assignment = ['ImplicitConvNet2DKernel', 'TruncatedFourierExpansion']
    kernel_params = {
        ...
        'TruncatedFourierExpansion': {
            'func': lambda x: torch.sin(2*np.pi*x/365.).to(device), 
            'T': 365., 
            'num_terms': 50, 
            'num_int_pts': 499, 
        }
...
def main(args):
    ...
    spearmanr, pearsonr, rmse, mae = calculate_stats(
        y_test_pred, 
        y_test_true, 
        data_save_path='./Results/CNN_Trunc_Fourier_temporal.pkl'
    )
    plot_pred_vs_true_vals(
        ...
        fig_save_path='./Figures/CNN_Trunc_Fourier_temporal.pdf', 
...
if __name__ == '__main__':
    arg_parser = argparse.ArgumentParser(description='Train a CNN-Truncated-Fourier ensemble model on remote sensing data.')
    ...
    arg_parser.add_argument('--latent_feature_dim', type=int, default=1)

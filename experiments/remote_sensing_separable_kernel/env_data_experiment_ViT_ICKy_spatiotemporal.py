...
def train_ick(data_generators, input_width, input_height, patch_size, latent_feature_dim, 
    ...
    kernel_assignment = ['ImplicitViTKernel', 'ImplicitNystromKernel', 'ImplicitNystromKernel']
    kernel_params = {
        # Modality 1: image
        'ImplicitViTKernel': {
...
def main(args):
    ...
    spearmanr, pearsonr, rmse, mae = calculate_stats(
        y_test_pred, 
        y_test_true, 
        data_save_path='./Results/ViT_ICKy_spatiotemporal.pkl'
    )
    plot_pred_vs_true_vals(
        ...
        fig_save_path='./Figures/ViT_ICKy_spatiotemporal.pdf', 
...
if __name__ == '__main__':
    arg_parser = argparse.ArgumentParser(description='Train a ViT-ICK model on remote sensing data.')

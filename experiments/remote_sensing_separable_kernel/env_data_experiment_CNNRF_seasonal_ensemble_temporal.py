...
def train_joint_model_ensemble(data_generators, input_width, input_height, in_channels, num_blocks, aug_feature_dim, 
                               lr, weight_decay, num_jobs, epochs, patience, verbose):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ensemble = [JointConvNet(
        input_width, 
        input_height,
        in_channels, 
        num_blocks, 
...
if __name__ == '__main__':
    arg_parser = argparse.ArgumentParser(description='Train a joint CNN-RF ensemble model on remote sensing data.')

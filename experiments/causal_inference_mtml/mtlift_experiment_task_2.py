...
def main(args):
    ...
    Y_train_task2 = train_df.to_numpy()[:,1][:,None]

    # Test data
    X_test = test_df.to_numpy()[:,3:]
    X_test_scaled = scaler_test.fit_transform(X_test)
    T_test = test_df.to_numpy()[:,2][:,None]
    Y_test_task2 = test_df.to_numpy()[:,1][:,None]

    # Initialize dataloaders
    data_train = [X_train_scaled, T_train]
    data_test = [X_test_scaled, T_test]
    data_generators_task2 = create_generators_from_data(
        data_train, Y_train_task2, data_test, Y_test_task2, train_batch_size=args.train_batch_size, test_batch_size=1000
    )
    ...
    trainer = CMICKEnsembleTrainer(
        model=ensemble,
        data_generators=data_generators_task2,
        optim=optim,
    ...
    print("AUUC for Y (conversion):", auuc)

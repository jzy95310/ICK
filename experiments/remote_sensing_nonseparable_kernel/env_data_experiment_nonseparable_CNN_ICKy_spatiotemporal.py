...
def train_ick(data_generators, input_width, input_height, in_channels, latent_feature_dim, 
    ...
    K_xst = ICK(
        kernel_assignment = ['ImplicitConvNet2DKernel', 'ImplicitNystromKernel', 'ImplicitNystromKernel'],
        kernel_params = {
            'ImplicitConvNet2DKernel':{
                'input_width': input_width,
                'input_height': input_height, 
...
def main(args):
    ...
    spearmanr, pearsonr, rmse, mae = calculate_stats(
        y_test_pred, 
        y_test_true, 
        data_save_path='./Results/Nonseparable_CNN_ICKy_spatiotemporal.pkl'
    )
    plot_pred_vs_true_vals(
        ...
        fig_save_path='./Figures/Nonseparable_CNN_ICKy_spatiotemporal.pdf', 

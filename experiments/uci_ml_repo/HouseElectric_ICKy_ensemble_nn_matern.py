...
#     df = df[list(df.columns[:2]) + list(df.columns[3:]) + [df.columns[2]]]
    ...
    t_train = scaler.fit_transform(t_train.reshape(-1,1)).squeeze()
    t_val = scaler.transform(t_val.reshape(-1,1)).squeeze()
    t_test = scaler.transform(t_test.reshape(-1,1)).squeeze()
